import { Comment, CommentDto, Lesson, LessonDetails, LessonDetailsDto, LessonDto } from "./model";
...
export const lessonDetailsDtoMap = (lessonDetailsDto: LessonDetailsDto): LessonDetails => {
  return {
...
export const commentDtoMap = (commentDto: CommentDto): Comment => {
  return {
    ...
    hasReplies: Boolean(commentDto.replies?.length && commentDto.replies.length > 0),
